...
w3 = 2*np.random.rand(r, q) - 0.5  # Layer 2
b3 = np.random.rand(r)

wOut = 2*np.random.rand(r) - 0.5  # Output Layer
...
for I in range(0, X_train.shape[0]): #loop in all the ids:
    ...
    z3 = ReLU_act(np.dot(w3, z2) + b3) #output layer 3
    y = sigmoid_act(np.dot(wOut, z3) + bOut) # Output of the Output layer

#2.2: Compute the output layer's error
    delta_Out =  (y-Y_train[I]) * sigmoid_act(y, der=True)
    
    #2.3: Backpropagate
    delta_3 = delta_Out * wOut * ReLU_act(z3, der=True) #Third Layer Error
    delta_2 = np.dot(delta_3, w3) * ReLU_act(z2, der=True) # Second Layer Error
    delta_1 = np.dot(delta_2, w2) * ReLU_act(z1, der=True) # First Layer Error
    
    # 3: Gradient descent 
    wOut = wOut - eta*delta_Out*z3  # Outer Layer
    bOut = bOut - eta*delta_Out

    w3 = w3 - eta*np.kron(delta_3, z2).reshape(r,q) # Hidden Layer 2
    b3 = b3 - eta*delta_3
...
def ANN_train(X_train, Y_train, p, q, r, k, eta):
    import numpy as np
    import matplotlib.pyplot as plt
    temp = np.random.choice(X_train.shape[0],k,replace=False)

    x = np.array(X_train[temp])
    y = np.array(Y_train[temp])
    ...
    w3 = 2*np.random.rand(r,q)-0.5 #Layer 3
    b3 = np.random.rand(r)
    ...
    for I in range(k):#range(0, X_train.shape[0]-1): #pick a small batch size of k #loop in all the bitcoin user ids:
        # 1: input the data 
        x = X_train[I]#temp[I]#X_train[I]
        print(x)
        ...
        z1 = ReLU_act(np.dot(w1, x) + b1) # output layer 1 
        z2 = ReLU_act(np.dot(w2, z1) + b2) # output layer 2
        z3 = ReLU_act(np.dot(w3, z2) + b3) # output layer 2
        y = sigmoid_act(np.dot(wOut, z3) + bOut) # Output of the Output layer
        ...
        w3 = w3 - eta*np.kron(delta_3, z2).reshape(r,q) # Hidden Layer 3
        b3 = b3 -  eta*delta_3
        ...
        # 4. Computation of the loss function
        mu.append((y-Y_train[I])**2) #(1/2)*(y-Y_train[I])**2
        vec_y.append(y) #y[0]
...
##    plt.show()
    ...
    return w1, b1, w2, b2, w3, b3, wOut, bOut, mu

w1, b1, w2, b2, w3, b3, wOut, bOut, mu = ANN_train(X_train, Y_train, p=10, q=7, r=7, k=25, eta=0.003)


#X_train, X_val, Y_train, y_val = train_test_split(X_train, Y_train, test_size=0.25, shuffle=True,random_state=1)

#compute predictions from trained ANN
def ANN_pred(X_test, w1, b1, w2, b2, w3, b3, wOut, bOut, mu):
    import numpy as np
...
predictions = ANN_pred(X_test, w1, b1, w2, b2, w3, b3, wOut, bOut, mu)
...
test_predictions = ANN_pred(X, w1, b1, w2, b2, w3, b3, wOut, bOut, mu)
